...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Create an empty set to store new nodes
        emptyset = set()
        # Loop through each of the actions
        for action in self.all_actions:
            # Create a new A node for each action
            newnode = PgNode_a(action)
            # If the prenode to the newnode is a subset of s_levels, add it to emptyset
            if newnode.prenodes.issubset(self.s_levels[level]):
                emptyset.add(newnode)
                # Loop through s_levels and if S nodes exist in prenodes, add S level node as a parent to newnode
                # and add newnode as a child to the S node
                for snode in self.s_levels[level]:
                    if snode in newnode.prenodes:
                        newnode.parents.add(snode)
                        snode.children.add(newnode)
        # Update a_levels by adding A nodes to the current A level
        self.a_levels.append(emptyset)
    ...
    def add_literal_level(self, level):
        ...
        # Create empty set to store S nodes
        emptyset = set()
        # Loop through A nodes in previous level
        for anode in self.a_levels[level - 1]:
            # Find effects of A nodes and store related S node to emptyset
            for snode in anode.effnodes:
                emptyset.add(snode)
                # Parents of S nodes are A nodes
                snode.parents.add(anode)
                # Children of A nodes are S nodes
                anode.children.add(snode)
        # Add the newly found S nodes to s_levels
        self.s_levels.append(emptyset)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Loop through effect_add actions for node_a1, if the effect is
        # also in effect_rem actions for node_a2, return
        # True for inconsistent effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Return True if actions in node_a1 match preconditions in node_a2
        for effect in node_a1.action.effect_add:
        ...
        # Return True if actions in node_a2 match preconditions in node_a1
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # If one precodition (parent) is mutex with the precondition of
        # another action, return True.

        # Loop through a1 nodes
        for a1_parent in node_a1.parents:
            # Loop through a2 nodes
            for a2_parent in node_a2.parents:
                # Compare nodes in a1 and a2
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # Check to see if both nodes have the same symbol
        same_symbol = node_s1.symbol == node_s2.symbol
        # Check to see if both nodes are positve or negative
        negation = node_s1.is_pos != node_s2.is_pos
        # Return bool results of same_symbol and negation
        return same_symbol and negation
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # Loop through states in node_s1 parents
        for s1_parent in node_s1.parents:
            # Loop through states in node_s2 parents
            for s2_parent in node_s2.parents:
                # If s1_parent is not mutex with s2_parent, return False
    ...
    def h_levelsum(self) -> int:
        ...
        # Loop through each goal in the problem
        for goal in goals:
            # The goal node (state) to reach
            node = PgNode_s(goal, True)
            # Create a list of different state levels and their level numbers
            s_levels_list = enumerate(s_levels)
            for level, s_nodes in s_levels_list:
                # If the goal node is in the current state, record the level
                if node in s_nodes:
                    level_sum += level
                    break
        return level_sum
